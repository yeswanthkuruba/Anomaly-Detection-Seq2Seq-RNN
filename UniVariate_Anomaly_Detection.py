# -*- coding: utf-8 -*-
"""
Created on Wed Jul  4 07:14:09 2018
...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun  8 17:08:08 2018
...
start = time.time()
data = pd.read_csv("univariate_data.csv",names=['timestamp','value'])
data["timestamp"] = pd.to_datetime(data["timestamp"], format = '%d-%m-%Y %H:%M')
data['weekday'] = data['timestamp'].dt.dayofweek
data['weekend'] = np.where(data['weekday']<5, 0, 1)
data['hour_day'] = data['timestamp'].dt.hour
enc = OneHotEncoder()
enc.fit(data[['weekend','weekday','hour_day']])  
time_encoder = enc.transform(data[['weekend', 'weekday', 'hour_day']]).toarray()
data_t = data[["value"]]
forecast_length = 1
...
with tf.Session() as sess:
    ...
    out = pd.DataFrame([actual_list,output_list,upper_bound_list,lower_bound_list,anomaly_score_list,anomaly_type_list])
    out_ = out.transpose()
    out_.columns = ["predicted", "value","upper_bound", "lower_bound", "anomaly_score", "anomaly_type"]      
    out_.to_csv("KPI_ANOMALY_DETECTION_Telefonica_09_06_all_168.csv", mode="a", header = False)
